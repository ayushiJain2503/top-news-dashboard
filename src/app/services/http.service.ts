...
export class HttpService {
  ...
  public getTopNews(category: string): Observable<News[]> {
    return this.http.get<News[]>(
      `/svc/topstories/v2/${category}.json?api-key=` + this.apiKey
    );
  }
   ...
   * @returns
   */
  public getSearchNews(
    filterText: string,
    page: number = 0
  ): Observable<NewsSearchData> {
    return this.http.get<NewsSearchData>(
      `/svc/search/v2/articlesearch.json?q=${filterText}&page=${page}&api-key=` +
        this.apiKey
    );
  }
}
