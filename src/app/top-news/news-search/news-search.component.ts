...
export class NewsSearchComponent implements OnInit, AfterViewInit {
    ...
    this.searchControl.valueChanges
      ...
      .subscribe((val) => {
        if (this.searchTextHistory.length) {
          this.filteredSearchHistory = this.searchTextHistory.filter((text) =>
  ...
  public handlePageEvent(event): void {
    ...
    this.newsApiService
      .getFilterNews(this.searchText, event?.pageIndex)
      .pipe(
        catchError((err) => {
          return throwError(err);
        })
  ...
  private addSearchHistory(text: string): void {
    const doesExist: boolean =
      this.searchTextHistory.filter(
        (val) => val.toLowerCase() === text.toLowerCase()
      ).length > 0;
    if (!doesExist) {
